...
const register = async (req, res) => {
    ...
    accountsDb.query("SELECT * FROM users WHERE email = ?", [email], async (error, result) => {
        ...
        if (result.length > 0) return res.status(400).json({ message: "Account already exists with that email" });
        
        const hashedPw = await bcrypt.hash(password, 10); //Hash plaintext password with a cost factor of 10, 2^10 = 1024 iterations
        accountsDb.query("INSERT INTO users (username, email, hashed_pw) VALUES (?, ?, ?);", [username, email, hashedPw], (error) => {
